...
function MenuCard(item, lang, i){
  ...
  return `
    <button class="k-card g-${item.group} anim" data-nav="menu:${item.id}" style="animation-delay:${0.2+i*0.06}s">
      ...
      ${item.group==='media' ? `<div class="tv" aria-hidden="true"><i class="wave"></i><i class="wave w2"></i><i class="wave w3"></i><div class="eq">${'<b></b>'.repeat(14)}</div></div><span class="live"><i></i>${T('live', lang)}<em id="kTc">--:--:--</em></span>` : ''}
